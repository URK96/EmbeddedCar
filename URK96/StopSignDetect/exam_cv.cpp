...
DriveLine OpenCV_hough_transform(unsigned char* srcBuf, int iw, int ih, unsigned char* outBuf, int nw, int nh, int* mode)
{
	...
	for (i = 0; i < iplImage->height; i++)
	{
		...
		{
			if (
					((unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + HUE] >= SSR_lowH &&
                    (unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + HUE] <= SSR_highH &&
					(unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + SAT] >= SSR_lowS &&
					(unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + SAT] <= SSR_highS) &&
					(unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + VAL] >= SSR_lowV &&
					(unsigned char)iplImage->imageData[i*iplImage->widthStep + 3 * j + VAL] <= SSR_highV
				)
            {
    ...
    cv::HoughLines(contours, lines, 1, PI/180, 55);  // ��ǥ(vote) �ִ� ����
